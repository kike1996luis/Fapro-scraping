...
def _scrap_uf(response, month, day):
    try:
        ...
        month_data = soup.find("div", attrs={"id": f"mes_{MONTHS[int(month)]}",
                                             "class": "meses"})
        table = month_data.find('table', {'class':
                                'table table-hover table-bordered'})
        rows = [[el.text.strip() for el in row.find_all("td")]
        ...
        return {'message': 'Cannot obtain data from UF web',
                'success': False}
...
def _validate_date(date):
    try:
        ...
        if format < datetime(2013, 1, 1):
            return {'message': 'The minimum date is 01/01/2013',
                    'success': False}
...
def get_uf_value(date: str):
    ...
    response = requests.get("https://www.sii.cl/valores_y_fechas/uf/uf"
                            + year + ".htm")
    if response.status_code != 200:
        return {'success': False, 'status': response.status_code,
                'message': 'UF web got unexpected state'}
